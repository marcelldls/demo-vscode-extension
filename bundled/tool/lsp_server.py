...
LSP_SERVER = server.LanguageServer(
    name="Demo vscode extension", version="<server version>", max_workers=MAX_WORKERS
)
...
def completions(params: lsp.CompletionParams):
    return [lsp.CompletionItem(label=f"{_func} ()") for _func in FUNCTIONS]

def get_word(line: str, position: int) -> str:
    try:
        front_split = line[:position].split()
        back_split = line[position:].split()
        word = front_split[-1]+back_split[0]
        return word
    except IndexError:
        return ""

@LSP_SERVER.feature(lsp.TEXT_DOCUMENT_HOVER)
def hover(ls: server.LanguageServer, params: lsp.HoverParams):
    pos = params.position
    ...
    word = get_word(line, pos.character)
...
def _run_tool_on_document(
    ...
    if use_path:
        ...
        with utils.substitute_attr(sys, "path", sys.path[:]):
            try:
                # TODO: `utils.run_module` is equivalent to running `python -m demo-vscode-extension`.
